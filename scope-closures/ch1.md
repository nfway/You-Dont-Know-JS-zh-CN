...
JS 引擎要想知道 _next 语句_ 将声明一个同名的块作用域变量（`greeting`）会出错，唯一的办法就是 JS 引擎已经在之前处理过这段代码，并设置了所有作用域及其变量关联。只有在执行前解析程序，才能准确地完成作用域和声明的处理。
...
(我在攻读计算机科学学位时第一次学习编译器理论，当时我们学习的术语 "LHS"（又称*目标*）和 "RHS"（又称*源*）分别表示这些角色。正如你可能从 "L" 和 "R" 中猜到的，这两个缩写词的意思是 "Left-Hand Side" 和 "Right-Hand Side"，就像 `=` 赋值运算符的左侧和右侧。不过，赋值目标和赋值源并不总是出现在 `=` 的左侧或右侧，所以用 _目标_ / _源_ 而不是 _左_ / _右_ 来表示可能更清楚）。
