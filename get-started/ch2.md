...
主要是围绕错误处理来理解。由于 JS 将文件视为程序，一个文件可能会失败（在解析/编译或执行过程中），这不一定会阻止下一个文件的处理。很明显，如果你的应用程序依赖于五个 .js 文件，而其中一个文件失败了，整个应用程序可能最多只能部分运行。重要的是，要确保每个文件都能正常工作，并且在任何可能的范围内，尽可能优雅地处理其他文件的故障。
